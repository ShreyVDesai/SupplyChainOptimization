...
from ML_Models.experiments.scripts.sarima_adf import (
    preprocess_data,
...
def train_sarima(csv_path, years=3):
    """
    Calls adf.py to get only the last `years` years of data, trains SARIMA, and logs experiment in MLflow.
    """
    mlflow.set_experiment("SARIMA_Demand_Forecasting")

    with mlflow.start_run(nested=True):
        # Get preprocessed data for last `years` years
        df = preprocess_data(csv_path, years=years)
        ...
        for m in seasonal_periods:
            try:
                print(f"Trying seasonal period m={m}...")
                model_candidate = auto_arima(
                    y, seasonal=True, m=m, trace=True, stepwise=True, d=2, D=1
                )
        ...
        print(
            f"Best Order: {best_order}, Seasonal Order: {best_seasonal_order}, Best m: {best_m}"
        )
        ...
        if len(y) >= forecast_steps:
            ...
            print(
                f"MAE: {mae:.4f}, MSE: {mse:.4f}, RMSE: {rmse:.4f}, MAPE: {mape:.2f}%"
            )
...
if __name__ == "__main__":
    csv_path = (
        "C:/Users/shrey/Projects/SupplyChainOptimization/data/sales_data.csv"
    )
