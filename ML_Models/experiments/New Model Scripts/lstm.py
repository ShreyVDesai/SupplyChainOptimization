# MODEL PREDICTION
# LSTM: 7-Day Demand Forecast per Product with RMSE & MAPE, and overall Average RMSE & MAPE.
...
def train_lstm_model(product_data):
    ...
    # Normalize data
    scaler = MinMaxScaler()
    product_data['Total Quantity'] = scaler.fit_transform(product_data[['Total Quantity']])
    ...
    val_data = product_data.iloc[train_size:train_size + val_size]['Total Quantity'].values
    test_data = product_data.iloc[train_size + val_size:]['Total Quantity'].values
    ...
    return {'rmse': rmse, 'forecast': forecast}
...
def main():
    ...
    print("\n--- Overall Model Performance ---")
    print(f"Average RMSE: {avg_rmse:.2f}" if not np.isnan(avg_rmse) else "Average RMSE: N/A")
    print(f"Average MAPE: {avg_mape:.2%}" if not np.isnan(avg_mape) else "Average MAPE: N/A")
    print(f"Average R² Score: {avg_r2:.3f}" if not np.isnan(avg_r2) else "Average R² Score: N/A")
