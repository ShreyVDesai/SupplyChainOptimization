...
class TestUtils(unittest.TestCase):
    ...
    @patch('scripts.utils.pl.read_excel')
    @patch('scripts.utils.storage.Client')
    def test_empty_dataframe_raises_value_error_load(self, mock_storage_client, mock_read_excel, mock_logger):
        # Setup
    ...
    @patch('scripts.utils.logger')
    def test_load_csv_success(self, mock_logger, mock_read_csv, mock_storage_client, mock_setup_gcp_credentials):
        # Setup
        bucket_name = "test-bucket"
        file_name = "test.csv"
    ...
    @patch('scripts.utils.logger')
    def test_load_json_success(self, mock_logger, mock_read_json, mock_storage_client, mock_setup_gcp_credentials):
        # Setup
        bucket_name = "test-bucket"
        file_name = "test.json"
    ...
    @patch('scripts.utils.storage.Client')
    @patch('scripts.utils.logger')
    def test_unsupported_file_type(self, mock_logger, mock_storage_client, mock_setup_gcp_credentials):
        # Setup
        bucket_name = "test-bucket"
        file_name = "test.txt"
    ...
    @patch('scripts.utils.pl.read_csv')
    @patch('scripts.utils.logger')
    def test_empty_dataframe_raises_value_error(self, mock_logger, mock_read_csv, mock_storage_client, mock_setup_gcp_credentials):
        # Setup
        bucket_name = "test-bucket"
        file_name = "test.csv"
    ...
    @patch('scripts.utils.pl.read_csv')
    @patch('scripts.utils.logger')
    def test_empty_dataframe_raises_exception(self, mock_logger, mock_read_csv, mock_storage_client, mock_setup_gcp_credentials):
        # Setup
        bucket_name = "dummy-bucket"
        file_name = "dummy.csv"
        dummy_bytes = b"col1,col2\n"  # header only, no data
    ...
    @patch('scripts.utils.pl.read_csv')
    @patch('scripts.utils.logger')
    def test_csv_read_error(self, mock_logger, mock_read_csv, mock_storage_client, mock_setup_gcp_credentials):
        # Setup
        bucket_name = "dummy-bucket"
        file_name = "dummy.csv"
    ...
    @patch('scripts.utils.pl.read_json')
    @patch('scripts.utils.logger')
    def test_json_read_error(self, mock_logger, mock_read_json, mock_storage_client, mock_setup_gcp_credentials):
        # Setup
        bucket_name = "dummy-bucket"
    ...
    @patch('scripts.utils.pl.read_excel')
    @patch('scripts.utils.logger')
    def test_excel_read_error(self, mock_logger, mock_read_excel, mock_storage_client, mock_setup_gcp_credentials):
        # Setup
        bucket_name = "dummy-bucket"
        file_name = "dummy.xlsx"
    ...
    def test_env_not_set(self, mock_logger):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            # Assert
            self.assertEqual(os.environ.get("GOOGLE_APPLICATION_CREDENTIALS"), "/app/secret/gcp-key.json")
            mock_logger.info.assert_called_with("Set GCP credentials path to: /app/secret/gcp-key.json")
    ...
    def test_setup_gcp_credentials_exception(self, mock_logger):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("os.environ.get", side_effect=Exception("test error")):
                ...
                self.assertTrue(
                    any("Error setting up GCP credentials:" in str(arg) 
    ...
    def test_env_already_set(self, mock_logger):
        # Test
        with patch.dict(os.environ, {"GOOGLE_APPLICATION_CREDENTIALS": "/app/secret/gcp-key.json"}, clear=True):
            setup_gcp_credentials()
            # Assert
            self.assertEqual(os.environ.get("GOOGLE_APPLICATION_CREDENTIALS"), "/app/secret/gcp-key.json")
            mock_logger.info.assert_called_with("Using existing GCP credentials from: /app/secret/gcp-key.json")
    ...
    @patch('scripts.utils.storage.Client')
    @patch('scripts.utils.logger')
    def test_upload_csv_success(self, mock_logger, mock_storage_client, mock_setup_gcp_credentials):
        bucket_name = "test-bucket"
        ...
        with patch.object(df, "write_csv", return_value=csv_content) as mock_write_csv:
            ...
            mock_logger.info.assert_any_call("Upload successful to GCS. Blob name: %s", destination_blob_name)
    ...
    @patch('scripts.utils.storage.Client')
    @patch('scripts.utils.logger')
    def test_upload_json_standard_success(self, mock_logger, mock_storage_client, mock_setup_gcp_credentials):
        bucket_name = "test-bucket"
        destination_blob_name = "data.json"
        df = pl.DataFrame({"a": [1, 2], "b": [3, 4]})
        json_content = '{"a": [1, 2], "b": [3, 4]}'
        with patch.object(df, "write_json", return_value=json_content) as mock_write_json:
            mock_blob = MagicMock()
            mock_blob.upload_from_string = MagicMock()
            mock_bucket = MagicMock()
            mock_bucket.blob.return_value = mock_blob
            ...
            mock_blob.upload_from_string.assert_called_once_with(json_content, content_type="application/json")
            mock_logger.info.assert_any_call("JSON data uploaded successfully using write_json")
    
    
    @patch('scripts.utils.setup_gcp_credentials')
    @patch('scripts.utils.storage.Client')
    @patch('scripts.utils.logger')
    def test_unsupported_file_extension(self, mock_logger, mock_storage_client, mock_setup_gcp_credentials):
        # Setup
        bucket_name = "test-bucket"
    ...
    def test_upload_csv_success_upload_to_gcs_(self, mock_logger, mock_storage_client, mock_setup_creds):
        # Setup
    ...
    def test_upload_to_gcs_bucket_error(self, mock_logger, mock_storage_client, mock_setup_creds):
        ...
        self.assertTrue(
            any("Bucket error" in str(arg) for call_args, _ in mock_logger.error.call_args_list for arg in call_args),
