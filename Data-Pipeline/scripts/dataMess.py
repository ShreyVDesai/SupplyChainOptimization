...
def mess_up_data(input_file, output_file):
    ...
    for col, pct in missing_values.items():
        if col in df.columns:
            df.loc[df.sample(frac=pct).index, col] = np.nan  # Introduce missing values globally
    ...
    if 'Date' in df.columns:
        ...
        df.loc[date_indices, 'Date'] = df.loc[date_indices, 'Date'].apply(
            lambda x: x.strftime('%d/%m/%Y') if pd.notna(x) and random.random() > 0.5 else x.strftime('%m-%d-%Y') if pd.notna(x) else x
        )
    ...
    for i in range(num_chunks):
        ...
        # Duplicate records
        duplicate_rows = chunk.sample(frac=0.01, random_state=42)  # 1% duplicate records
        chunk = pd.concat([chunk, duplicate_rows], ignore_index=True)
        
        if 'Product Name' in chunk.columns:
            chunk.loc[chunk.sample(frac=0.03).index, 'Product Name'] = chunk['Product Name'].astype(str).apply(
                lambda x: x.replace("a", "@") if random.random() > 0.5 else x.replace("o", "0")
        ...
        if 'Cost Price' in chunk.columns:
            chunk.loc[chunk.sample(frac=0.01).index, 'Cost Price'] = -abs(chunk['Cost Price'].mean(skipna=True))  # 1% negative cost prices
        
        # Introduce negative and zero values in Quantity column
        if 'Quantity' in chunk.columns:
            chunk['Quantity'] = pd.to_numeric(chunk['Quantity'], errors='coerce')  # Convert to numeric before applying transformations
            chunk.loc[chunk.sample(frac=0.02).index, 'Quantity'] = -abs(chunk['Quantity'].mean(skipna=True))  # 2% negative quantities
            chunk.loc[chunk.sample(frac=0.01).index, 'Quantity'] = 0  # 1% zero quantities
        
        # Introduce invalid store locations
        if 'Store Location' in chunk.columns:
            chunk.loc[chunk.sample(frac=0.02).index, 'Store Location'] = str(random.randint(1000, 9999))  # 2% numeric store locations
        
        # Introduce logical inconsistencies
        if 'Cost Price' in chunk.columns and 'Quantity' in chunk.columns:
            inconsistent_indices = chunk.sample(frac=0.02).index  # 1% logical inconsistencies
            chunk.loc[inconsistent_indices, 'Cost Price'] = 0  # Cost Price is zero
            chunk.loc[inconsistent_indices, 'Quantity'] = random.randint(1, 10)  # Quantity is non-zero
    ...
    final_df.to_excel(output_file, sheet_name='Full_Data', index=False, engine='openpyxl')
